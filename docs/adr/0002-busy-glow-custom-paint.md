...
- **入场窗口乘进渐变的 alpha。** 已长出的那段弧（仍是以底边中点为中心的单段弧）按弧长位置算出每个色标的可见度，直接乘进光晕和边线两张渐变的 alpha；不再把路径拆成主体加二十段渐隐短段逐段描，也不再整块填一遍 `DestinationIn` 遮罩。入场期间色标加密一倍，渐隐两端才不会被色标间距截成台阶。弧的实心部分要多伸出一截渐隐长度（伸到半周长再加 `TAPER_FRACTION`）：只伸到半周长时，两端渐隐在顶部汇合处还留着一段淡色缺口，进度到 1 的那一帧才被一次补满，看着像"啪"地合拢。
...
- **配色保持原来的蓝、靛、紫、粉、橙。** 试过 Apple 智能的粉彩取色，深色下加性合成后发白、颜色辨识度反而下降，没有采用。浅色配方仍是提饱和 ×1.30、明度 ×0.68。
