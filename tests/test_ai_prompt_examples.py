...
class HolidayCacheTest(TestCase):
    setUp = test_ai_admin.AIAdminTest.setUp
    ...
    def testAFailedFetchKeepsThePreviouslyKnownHolidays(self):
        ai_markdown._connect().close()
        yesterday = (
            datetime.now(ai_markdown.TIMEZONE).date() - timedelta(days=1)
        ).isoformat()
        with closing(ai_markdown._connect()) as database:
            database.execute(
                "INSERT INTO settings(key, value) VALUES ('holiday_cache', ?)",
                (json.dumps({"refreshed": yesterday, "dates": ["2026-10-01"]}),),
            )
            database.commit()
        ...
        self.assertIn("2026-10-01", ai_markdown._cachedHolidays())
