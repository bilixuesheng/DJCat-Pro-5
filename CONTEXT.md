...
MainWindow 的缩放命中宽度读取 Windows 的 DPI 感知系统指标 `SM_CXSIZEFRAME/SM_CYSIZEFRAME + SM_CXPADDEDBORDER`，并额外增加 3 个随 DPI 缩放的逻辑像素作为触控余量；窗口切换屏幕时重新计算，非 Windows 环境或系统指标不可用时回退为 11 个逻辑像素。不得继续任意放大该范围，否则顶部和右侧命中带会覆盖标题栏最小化、最大化与关闭按钮。
