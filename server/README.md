...
部署前先备份 SQLite 数据库，并为每个已启用的 x86_64/arm64 Package 准备对应 ZIP 的 SHA-256。新版首次连接旧库会增加校验字段；哈希为空的安装包会被安全暂停，必须在恢复对外服务前从后台补齐并核对。随后重启 Gunicorn，分别在两个站点执行 `nginx -t`，通过后执行 `systemctl reload nginx`。确认 `18080` 只监听 `127.0.0.1`。

客户端发版顺序固定为：先部署兼容服务端并补齐 Package 哈希，再构建并上传两种架构的安装包及 `.sha256`，确认 Release 六个附件完整且摘要校验通过，最后才把 beta 更新接口切换到新版本。不要先更新 beta API，否则旧服务端或缺失资产会让客户端拿到不可安装的更新。
